...
function onGettingFile(f, item) {
  ...
  if (!file) {
    if (item && item.name) {
      ...
      header.querySelector('p').innerHTML = 'No se encontraron grabaciones.';
      header.querySelector('span').innerHTML = '';
    }
    return;
  }
  ...
  if (file.name && (file.name.indexOf('.mp3') !== -1 || file.name.indexOf('.wav') !== -1 || file.name.indexOf('.ogg') !== -1)) {
    video.style.background = 'url(images/audio.png) no-repeat center center';
  ...
  var html = '<a style="color: white;" href="' + file.url + '" download="' + file.name + '">Descargar Archivo</a>.';
...
document.querySelector('#btn-recordings-list').onclick = function (e) {
  ...
  if (btnRecordingsListDropDown.className === 'visible') {
    ...
    DiskStorage.GetFilesList(function (list) {
      if (!list.length) {
        btnRecordingsListDropDown.className = '';
        alert('Tu no tienes grabaciones.');
        return;
      }
      ...
      list.forEach(function (item) {
        ...
        div.querySelector('.cross-icon').onclick = function (e) {
          ...
          if (!window.confirm('Estas seguro que desea eliminar permanentemente la grabación?')) {
            return;
          }
        ...
        div.querySelector('.edit-icon').onclick = function (e) {
          ...
          var newFileName = prompt('Ingrese el nuevo nombre ', item.display) || item.display;
